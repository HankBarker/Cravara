...
local bench=loadpath('art/forest-pass5/workbench-source.png')
save(crop(bench,0,0,bench.width,bench.height,32,24,1),'workbench')
local chest=loadpath('art/forest-pass5/chest-source.png')
local base=nil
local strip=Image(48,20,ColorMode.RGB)
for i=0,2 do
 local frame=crop(chest,i*chest.width/3,0,(i+1)*chest.width/3,chest.height,16,20,1)
 if i==0 then base=Image(frame) else
  for y=15,19 do for x=0,15 do frame:drawPixel(x,y,base:getPixel(x,y)) end end
 end
 save(frame,'chest_'..i);strip:drawImage(frame,Point(i*16,0))
end
save(strip,'chest')
local fire=loadpath('art/forest-pass5/campfire-source.png')
local flames=Image(112,24,ColorMode.RGB)
local fixed=nil
for i=0,3 do
 local x0=math.floor(i*fire.width/4)
 local x1=math.floor((i+1)*fire.width/4)
 local frame=crop(fire,x0,0,x1,fire.height,28,24,1,nil,true)
 if i==0 then fixed=Image(frame) else
  for y=0,23 do for x=0,27 do
   if x<8 or x>19 or y>17 then frame:drawPixel(x,y,fixed:getPixel(x,y)) end
  end end
 end
 save(frame,'campfire_'..i);flames:drawImage(frame,Point(i*28,0))
end
save(flames,'campfire')
local atlas=loadpath('game/WorldObjects/Images/Objects.png')
save(crop(atlas,541,225,580,255,32,32,2),'mushroom_item')
local tent=loadpath('game/Forest/art/v2/tent.png')
save(crop(tent,0,0,tent.width,tent.height,32,32,2),'tent_item')
local vials=loadpath('art/forest-pass5/vials-source.png')
for i,name in ipairs({'crystal_flask','water_flask','mushroom_potion'}) do
 save(crop(vials,math.floor((i-1)*vials.width/3),0,math.floor(i*vials.width/3),vials.height,32,32,2),'item-'..name)
end
local cooked=loadpath('art/forest-pass5/cooked-fish-source.png')
save(crop(cooked,0,0,cooked.width,cooked.height,32,32,2),'item-cooked_fish')

save(crop(bench,0,0,bench.width,bench.height,32,32,2),'item-workbench')
save(crop(chest,0,0,chest.width/3,chest.height,32,32,2),'item-chest')
local hearth=Image{fromFile=out..'campfire_0.png'}
save(crop(hearth,0,0,hearth.width,hearth.height,32,32,2),'item-campfire')
