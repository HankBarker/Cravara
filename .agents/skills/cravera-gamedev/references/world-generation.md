...
## Current forest slice (2026-09-15)

- `ForestWorld` stores `floors`, solid `props`, and `roofs` independently. Legacy saved `placed` floor entries migrate on restore; do not reintroduce mutually exclusive placement for these layers. Roof artwork and target bounds occupy the same 16px cell.
- Rich ground textures are baked to a static 1792px SubViewport surface, refreshed on terrain edits/restore. Drawing all 12,544 textured cells in the live canvas caused about 31,800 draw calls; the cached implementation renders the same detail with hundreds. Verify actual bucket-edited image pixels as well as world data.
- Sun shadows cache local silhouette geometry by prop art variant and quantized sun phase, then draw with world translation. Repeating polygon unions and triangulation every frame was a measured bottleneck.
- Anchor sun projections to opaque artwork contact, not transparent image bounds. Native silhouette occlusion handles ground/actors; a separate restrained prop-surface light prevents 2D sprites from blacking themselves out. This is stylized lighting, not 3D normals.
...
# Forest gardening and worker contacts (2026-09-16)

Gardening.gd stores tilled plots separately from solid props and persists seed, growth and watered state. Use full16px tile versus actual structure footprint intersection: checking only an anchor/cursor point allows broad furniture to cover neighboring crops. Roofing remains overhead and must not erase soil. Garden growth advances only active process time; watering exchanges a filled bucket for an empty one in place. Worker harvesting is natural-only, applies durability hits, and returns cargo dictionaries without spawning player drops. Never both return cargo and call the normal loot-drop path.

Godot PhysicsRayQueryParameters2D.exclude returns an array copy. Assemble a local Array[RID], append rider/mount exclusions there, then assign the property once. Direct ray.exclude.append() silently leaves the query unchanged; downward mounted-arrow tests caught this. Save compatibility tests must compare JSON numeric quantities after int conversion, because dictionary equality distinguishes loaded floats from runtime integers.
