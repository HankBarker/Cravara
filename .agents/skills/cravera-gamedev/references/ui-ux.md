...
## Forest playtest integration notes (2026-09-15)

- E is a tap/hold interaction: release before the threshold mounts/dismounts or interacts; holding opens companion commands without triggering the tap action on release. Q near a bonded creature targets that creature; Q away opens group orders.
- Input regressions must exercise both the viewport and Input singleton. For graphical aim, synchronize the viewport mouse position; pushed GUI events alone can leave the world mouse position stale. Test a held click while closing a modal, not only isolated button signals.
- A respawning player must block HUD shortcuts as well as world input. The forest's five-second death flow explicitly reenables physics when returning from the legacy Dead state.
- Corner shortcut visibility controls the three clickable charms only. Tab/K/P bindings and the hotbar remain available.
- Fishing is an exclusive modal: `FishingPanel` consumes Space press/release and other keyboard/wheel events before HUD `_unhandled_input` can switch hotbar slots. Guarding HUD `_input` alone does not block number-key action bindings in `_unhandled_input`.
- For aim-sensitive graphical tests at 120fps, await a physics frame and process frame before converting world to canvas coordinates. Camera updates after mounted seating can change the transform; synchronize OS warp plus synthetic input and verify measured world mouse position before clicking. Keep strict aim assertions, recomputing coordinates if still settling.
- `FishingController` is explicitly PAUSABLE beneath the ALWAYS-processing forest session. A cast persists neither through pause/death/accepted damage nor through a selected-rod change. Rewards are atomic: one item on success, or a DroppedItem when full, with no rod cost on cancellation/failure. `ForestFishingTest.tscn` covers these paths with actual Space input.
