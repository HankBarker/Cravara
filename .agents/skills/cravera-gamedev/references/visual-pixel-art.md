...
- **PixelLab — WIRED IN as the `mcp__pixellab__*` MCP server.** Read
  **[`pixellab-sprites.md`](pixellab-sprites.md)** for the generation playbook, the style
  contract, the 40-generation trial budget guard, and `tools/pixellab_import.py` (the
  quantize-gate + SpriteFrames importer). Summary of what it does:
  game-dev focused: upload a **concept + reference sprite** to generate **consistent
...
## Cravera canonical pipeline (IMPLEMENTED)

The visual-cohesion pipeline above is now concrete in the repo. This is the source of truth.

**Master palette — `art/palettes/cravera_master.{hex,gpl}`.** A hand-tuned ~39-color, grassland-first
palette with hue-shifted ramps (shadows cool toward blue/purple, highlights warm toward yellow) for
foliage greens, earth/wood browns, stone/grey, two dino-skin hues, water blues, torch/fire,
blood/danger red, bone/parchment, and night blue/purples for the day-night tint. It expands as new
biomes land. Load `cravera_master.gpl` in Aseprite (`Palette ▸ Load Palette`). **Every asset — AI
output, asset-pack art, hand-drawn — must be indexed to this palette before import.** `.hex` is the
Lospec format consumed by the quantize gate; `.gpl` is the Aseprite/GIMP working palette.

**PixelLab importer — `tools/pixellab_import.py`.** Takes a PixelLab spritesheet export (zip or
URL), runs it through the quantize gate below, and emits `game/Sprites/<Name>/<name>_sheet.png`
plus a Godot `SpriteFrames` `.tres` using Cravera's `walk_<dir>`/`bite_<dir>`/`death` naming —
then runs Godot's import pass and load-tests the resource. This is the only supported route
from AI-generated art into the game. See `pixellab-sprites.md`.

**The quantize gate — `tools/quantize_to_palette.py` (Pillow only).** Run on EVERY incoming asset; it
snaps each pixel to the nearest master-palette color (Euclidean distance in sRGB), the scriptable
equivalent of Aseprite's "Color Mode ▸ Indexed (nearest)". Canonical command:

```sh
python tools/quantize_to_palette.py path/to/sprite.png --out-dir game/Sprites/...
```

Crisp-sprite by default: sub-threshold alpha → fully transparent, the rest → opaque palette colors.
Flags: `--palette` (override the .hex), `--in-place`, `--alpha-threshold N` (default 128),
`--keep-alpha` (preserve original alpha), `--dither` (Floyd-Steinberg; off by default for sharp
pixels). Accepts multiple paths and globs (`"packs/*.png"`); prints a before/after unique-color
summary.

**Integer scaling.** `display/window/stretch/scale_mode = "integer"` is being set in `project.godot`:
480→1920 and 270→1080 are both exactly 4×, so whole-number upscaling is free and stays crisp.

**Canonical rules (hold these):** 16px tile/object grid; 32px footprint only for hero dinos; one PPU
everywhere; **never** non-integer scale and **never** off-90° rotation of pixel sprites.
