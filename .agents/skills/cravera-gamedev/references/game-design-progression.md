...
## Current forest override (September 2026, pass 5)

The playable ForestPlayer deliberately supersedes the historical stamina guidance below. Only vitality and hunger are player-facing. Legacy stamina fields remain readable for old saves and shared FSM compatibility, but cannot prevent running or attacking. Hunger drains 0.012/s idle, 0.035/s walking and 0.16/s sprinting; tool swings add 0.1 exertion. Item.food_satiation_seconds delays drain, consumed at 0.5/1/2 seconds per idle/walk/sprint second. Roasted meat fills hunger and supplies 180 seconds of walking fullness. Balance is a playtest hypothesis, not a universal requirement.

Food effects carry item identity and refresh instead of stacking identical healing. Tonic effects work without hunger. Reusable consumables use Item.consumed_container_id and ForestPlayer.consume_slot to replace an unstackable dose atomically in its exact inventory/chest slot. Both world right-click and SlotUI use this same transaction. Death saves serialize the same reset effects/fullness as actual respawn. Journey drops persist separately from world props so full-inventory fishing rewards survive a reload; restoration replaces existing drops rather than adding duplicates.

Meaningful regressions: ForestRootPass5 covers activity rates, meal duration, legacy saves, potion vessel conservation, death-save parity and uncollected drops. ForestFishingTest drives actual Space input through win/loss and modal/damage cancellation. Do not restore energy costs from the older design examples below.
