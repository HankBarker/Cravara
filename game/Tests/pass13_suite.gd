...
func _skills() -> void:
	...
	check(sk.level("combat") == 2 and int(sk.points.combat) == 2, "doing it levels it up, with two star points")
	...
	check(not sk.learn("sweep_arc") and sk.blocked("sweep_arc").begins_with("Learn Blade-sense"), "a star hangs from the root (%s)" % sk.blocked("sweep_arc"))
	check(sk.learn("blade_sense") and sk.learn("sweep_arc") and sk.has("sweep_arc"), "the root, then a star along the guard")
	...
	# Level 5: the old callings are stars now, and both of a pair can be lit.
	for i in 3: sk.gain("combat", float(Skills.TO_NEXT[sk.level("combat") - 1]))
	check(sk.level("combat") == 5 and int(sk.points.combat) == 6, "level 5: eight points earned, two spent")
	...
	add_child(copy)
	copy.restore(saved)
	check(copy.level("combat") == 5 and copy.has("sweep_arc") and copy.has("duelist") and int(copy.points.combat) == int(sk.points.combat), "skills survive a save")
	...
	check(copy.has("sweep_arc") and copy.has("duelist") and int(copy.points.combat) == 6, "an old journey's calling is a star, its points made up (%d)" % int(copy.points.combat))
	copy.queue_free()
