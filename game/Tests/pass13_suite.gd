...
func _skills() -> void:
	...
	check(sk.level("combat") == 2 and int(sk.points.combat) == 1, "doing it levels it up, with a star point")
	check(is_equal_approx(sk.value("melee_damage"), 0.006), "each level a small boost (+0.6% melee)")
	var shape_before: float = float(keeper.blow_shape("sweep").arc)
	check(not sk.learn("sweep_arc") and sk.blocked("sweep_arc").begins_with("Needs Combat"), "a star waits for its level (%s)" % sk.blocked("sweep_arc"))
	check(sk.learn("blade_sense"), "the root first")
	# Pass 15: the long road. Level 6 opens the guard's first stars.
	while sk.level("combat") < 6: sk.gain("combat", float(Skills.TO_NEXT[sk.level("combat") - 1]))
	check(sk.learn("sweep_arc") and sk.has("sweep_arc"), "then a star along the guard")
	check(float(keeper.blow_shape("sweep").arc) > shape_before, "Wide Arc widens the sweep")
	check(not sk.learn("sweep_cleave") and sk.blocked("sweep_cleave") != "", "a later star waits (%s)" % sk.blocked("sweep_cleave"))
	# The old callings are stars now, and both of a pair can be lit.
	while sk.level("combat") < 23: sk.gain("combat", float(Skills.TO_NEXT[sk.level("combat") - 1]))
	check(sk.level("combat") == 23 and int(sk.points.combat) == int(sk.earned("combat")) - 2, "level 23: every point earned, two spent")
	check(sk.learn("stab_quick") and sk.learn("riposte") and sk.learn("duelist") and sk.learn("brawler"), "Duelist and Brawler both (nothing is one-or-the-other)")
	var kills := {"raptor": Skills.kill_xp({"species": "raptor"}), "rex": Skills.kill_xp({"species": "rex"}), "sick rex": Skills.kill_xp({"species": "rex", "crystal": 2})}
	check(float(kills.rex) > float(kills.raptor) * 5.0 and float(kills["sick rex"]) > float(kills.rex), "a rex teaches far more than a raptor, a Crystalback more still (%s)" % kills)
	var saved: Dictionary = sk.serialize()
	var copy: Node = Skills.new()
	add_child(copy)
	copy.restore(saved)
	check(copy.level("combat") == 23 and copy.has("sweep_arc") and copy.has("duelist") and int(copy.points.combat) == int(sk.points.combat), "skills survive a save")
	# A pass-13 journey: perks and callings become stars; the points are made up.
	copy.restore({"levels": {"combat": 5}, "perks": ["sweep_arc"], "callings": {"combat:5": "duelist"}, "points": {"combat": 1}})
	# (Its XP re-counted on the long road: 670 of it makes level 5, four points, two lit.)
	check(copy.has("sweep_arc") and copy.has("duelist") and copy.level("combat") == 5 and int(copy.points.combat) == 2, "an old journey's calling is a star, its XP re-counted (level %d, %d points)" % [copy.level("combat"), int(copy.points.combat)])
	copy.queue_free()
...
func _count_new_hunters() -> String:
	var n := {}
	for c in get_tree().get_nodes_in_group("forest_creatures"):
		# (The living: a hunter killed out in the wild just before the save lies
		# there dead and isn't saved.)
		if c.species in ["deino", "utah", "sucho", "spino"] and not c.is_dead and not c.is_queued_for_deletion():
