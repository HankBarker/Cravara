...
func run() -> void:
	...
	# Pass 14: the gear is the pack's right edge; K toggles the pack like Tab.
	await key(KEY_K)
	check(not hud.is_open(),"K closes the open pack")
	await key(KEY_K)
	check(hud.equipment_panel.visible and hud.inventory_panel.visible,"K opens the pack with its gear")
	check(hud.armor_buttons.size()==9,"Gear has 9 places (head, body, legs, light, five trinkets)")
