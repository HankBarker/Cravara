...
func run():
	...
	var tail_hit:int=preload("res://Forest/creatures/DinoArt.gd").hit_frame(stego.art_key,"tail_swing","down")
	check(stego._work_swing_time>0 and stego._sprite.animation=="tail_swing_down" and stego._sprite.frame==tail_hit,"timber hit: back to the trunk, tail on its contact frame")
