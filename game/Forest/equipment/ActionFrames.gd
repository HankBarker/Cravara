...
const DURATIONS := {"axe":.48,"pickaxe":.56,"weapon":.32,"sword":.42,"thrust":.44,"bow_draw":.40,"bow_release":.24,"fishing_cast":.60,"fishing_reel":.64,"hoe":.56,"net":.52,"bucket":.64,"pet":.60,"pickup":.44,"eat":.72,"craft":.64,"place":.36,"interact":.30,"cheer":.80,"roll":.40}
static func duration(kind: String) -> float:
	return float(DURATIONS.get(kind,.48))
static func contact_ratio(kind: String) -> float:
	return .5 if kind == "weapon" else (.55 if kind in ["sword", "thrust"] else (.58 if kind in ["axe","pickaxe"] else .6))
