...
func use_at(target: Vector2, id: String) -> bool:
	...
	if id=="garden_hoe":
		...
		for prop in world.props.values():
			var footprint: Rect2=prop.get_collision_rect()
			if footprint.has_area() and tile.intersects(Rect2(prop.position+footprint.position,footprint.size)):
...
func _draw():
	for c in plots:
		...
		draw_style_box(_soil_style(bool(p.watered)),Rect2(origin+Vector2.ONE,Vector2(14,14)))
		for row in range(3):
			draw_line(origin+Vector2(3,4+row*4),origin+Vector2(13,3+row*4),Color("554332") if p.watered else Color("795a3c"),1)
		if p.seed=="": continue
...
func _soil_style(wet: bool) -> StyleBoxFlat:
	var style:=StyleBoxFlat.new()
	style.bg_color=Color("68503d") if wet else Color("947047")
	style.border_color=Color("4e4935")
